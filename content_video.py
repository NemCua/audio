...
def ai_split_scenes(content: str, groq_key: str, beeknoee_key: str = '') -> list[dict]:
    ...
    prompt = f'''You are a cinematic AI video director creating B-roll prompts for an AI video generator (Google Veo).

Read the content carefully and for EACH sentence/idea, write a cinematic video generation prompt that:
- Describes EXACTLY what should appear on screen for that moment
- Is vivid, specific, and visual — not abstract
- Uses cinematic language: camera angles, lighting, motion, atmosphere
- Can realistically be generated by AI (no text overlays, no real people's faces)

Examples of GREAT prompts:
- "Cinematic close-up of a massive black hole with glowing accretion disk spinning in deep space, dramatic lighting, 4K"
- "Slow motion neutron star collision sending gravitational waves rippling through spacetime visualization, cosmic scale"
- "A lone astronaut floating in orbit above Earth, sunlight reflecting off visor, Earth's curvature visible below"
- "Ancient Roman Colosseum at golden hour, crowd of gladiators in the arena, epic cinematic shot"
- "DNA double helix slowly rotating in blue bioluminescent light, microscopic zoom, scientific visualization"

Rules:
- duration: 5-8 seconds per scene
- prompt: 1-2 sentences, rich visual description in English
- Every prompt MUST be DIFFERENT and match the specific moment in the content
- Prefer dynamic motion: rotating, exploding, flowing, zooming, panning...
- NO text, NO subtitles, NO watermarks in prompt
- query field: 3-4 word summary (for logging only)

Return ONLY a JSON array, no explanation:
[{{"query": "short label", "prompt": "full cinematic prompt...", "type": "video", "duration": 6}}, ...]
